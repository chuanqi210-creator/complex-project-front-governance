...
If you are inside a downstream target project, resolve the Complex source before scanning the project. In this local environment, prefer `/Users/chuchenqidawang/Documents/complex-project-front-governance`. If that path is unavailable, look for sibling repos named `complex-project-front-governance` or `complex-project-continuous-governance`, then ask the user for the path. A target repo without a `Complex` directory only lacks a local copy; it does not prove Complex is unavailable.
...
Complex 权威来源优先使用：/Users/chuchenqidawang/Documents/complex-project-front-governance。请分开读取 Complex 协议源和目标项目材料：Complex 源读 README.md、protocol/current-state.md、docs/quickstart.md、protocol/core.md、templates、behavior cases 和 examples；目标项目再读它自己的 AGENTS.md、CONTEXT.md、状态文件、manifest、stage board 和代码。不要因为目标项目里没有 Complex 目录就说找不到 Complex；如果权威路径不可访问，再向我索要路径。
