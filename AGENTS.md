...
- Apply `human_intervention_drift_guard` inside decision rights: before asking the user, prove the issue is a goal, authorization, irreversible action, public voice, high-risk claim, external write, privacy, account, payment, publishing, or real-world responsibility boundary. If `next_route`, `round_goal`, state, or accessible materials already define a clear low-risk reversible next step, continue and record why user intervention was unnecessary.
- Use `context_pointer_first_intake`: when the user provides directories, files, links, exports, or material locations, read and summarize accessible materials yourself before asking for manual MD cleanup, copy-paste, or summaries. Ask for manual action only when access, permission, privacy, account, external-write, or real-world judgment boundaries block AI intake.
- Do not present same-session roleplay as independent review. For real independent review, use clean context, a separate thread/reviewer, read-only audit subagent, or a fact-ledger-only packet; otherwise label it as same-session diagnostic self-review.
...
- Prefer the filled golden examples in `docs/examples/` when teaching a new project how Runtime Kit records should look: evidence fill, model discovery, and independent review. Do not make users infer usage from empty templates alone.
...
- Classify new user input before it patches the plan: fact, preference, authorization, local correction, goal change, or noise/possible misleading. Only goal changes and authorization boundaries rewrite the mainline; local corrections patch the current round.
