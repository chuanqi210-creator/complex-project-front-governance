...
本协议用于复杂项目真正执行前。目标是让 AI 和用户先完成材料接收、上下文澄清、问题识别、能力搜索、主链收束、阶段路由、Goal/Plan、评分和最小闭环设计，再进入执行。
...
请按“复杂项目启动前置治理协议 v3”先完成 material intake、current_basis 判断、情景识别、行业最佳实践调查、阶段路由、评分和最小闭环设计。
...
如果用户明确说“后续阶段不必询问我”“连续完成前置治理”或类似指令，则进入连续治理模式。在连续治理模式下，AI 可以连续推进多个前置治理阶段，但仍必须遵守：
...
46.1-补充. `protocol_onboarding_comprehension_gate`：新项目要求读取 Complex、Auto Research、前置治理协议或类似强约束方法时，必须先安排协议理解时间，再开始业务产出。最低要求是读核心入口、阶段流程、能力发现、子代理/线程、交付拆分和人看版规则，并用 5-8 行说清本项目会采用哪些部分、不会展开哪些部分、为什么。若协议较长或项目高风险，可以让只读子代理专门做协议理解和标题/知识图谱式权重分配；但最终采用与取舍必须由主代理决定。
46.2. 当任务本身是“持续优化项目前置协议”时，必须把它当成一个真实项目，但只能使用 `continuous_optimization_meta_project_profile` 这种压缩形态治理：先恢复最新机器看版，区分真实缺口、用户偏好和工具层惯性，列出 2-3 个候选优化点，只收束一个最高杠杆主链，用现有评分体系判断是否继续，并至少跑一个 5-30 分钟真实小题或恢复链小题。该 profile 不是新的大目录，也不自动进入 verifier required。单一案例细节进入经验库；两个以上真实项目暴露同类失败模式，才考虑晋升为主协议规则；只有能被 verifier 稳定检查的通用规则，才考虑进入 required 核心字段。候选优化点必须标注 `promote_to_protocol`、`add_to_experience_library`、`reject` 或 `backlog`。
...
经验库索引位置：`/Users/chuchenqidawang/Documents/ai 科研/前置治理协议_真实项目经验库索引_20260622.md`。
...
77. **protocol_onboarding_underread_gap**：新项目要求读取 Complex、Auto Research 或前置治理协议时，模型没有先理解核心入口、阶段流程、能力发现、子代理/线程和交付拆分规则，就直接开始业务执行，导致“按协议推进”只停留在口头。
