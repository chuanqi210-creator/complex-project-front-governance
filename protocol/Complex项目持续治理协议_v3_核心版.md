...
Complex 也要防止把治理负担过早推回给用户。人工介入默认是成本和潜在漂移源；问用户前必须证明必要性。若下一步明确、低风险、可逆且材料可访问，AI 应按 `known_next_step_auto_execute_rule` 继续推进并记录理由，而不是询问“是否继续”；若用户给出目录、文件、链接或材料位置，默认按 `context_pointer_first_intake` 自行读取、拆分和归纳，只有不可访问、权限不足、隐私敏感、账号/付款/发布/外部写入或现实判断不可替代时才要求用户手动操作。
...
3. **划清决策权**：默认强自治+护栏；AI 处理可逆低副作用细节，目标/授权/不可逆/公开口径/高风险主张回问。回问用户前先过 `ask_user_necessity_gate`：能由 AI 低风险推进的，不把劳动、责任或判断甩回用户。
4. **选择一个最高杠杆问题**：本轮只抓最能降低返工或推动交付的主问题，不被局部材料缺口和工具兴趣劫持。
5. **用最轻有效动作验证**：可 no-op、读文件、检索、Loop 探针、子代理只读审计、工具烟测或直接执行；复杂度必须由不确定性和副作用驱动。若 state、`round_goal` 或 `next_route` 已给出明确、可逆、低副作用下一步，默认自动执行，不停在“要不要继续”的礼貌性确认上。
...
`human_intervention_drift_guard` 不是新的厚流程，而是第 3 步和第 5 步的共同约束：防 AI 越权，也防 AI 把可自行完成的低风险工作、上下文整理或连续推进过早转交给人。`ask_user_necessity_gate` 只回答一个问题：这件事是否真的属于主目标改变、授权、不可逆动作、公开口径、高风险主张、账号/API/付款/发布/外部写入、隐私或现实责任？若不是，默认 AI 继续推进，并在 `judgment.md` 或状态记录里写明 `unnecessary_user_intervention_reason`。

`independent_review_context_separation`：独立评审不能由同一会话里的角色扮演替代。若任务需要真正审查，应优先使用清上下文的新线程、只读子代理、不同模型/审查者，或至少只传事实账本和交付物，不传带倾向的完整聊天历史。同会话自评可以作为快速 diagnostic，但不得包装成独立评审结论。
...
| 少问我、能推进就继续、不要把活甩给我、已知 next_route 就继续、我给目录你自己读 | `ask_user_necessity_gate`、`known_next_step_auto_execute_rule` 和 `context_pointer_first_intake`，先判断人工介入是否必要；低风险可逆且材料可读时自动推进 |
...
| 独立评审、客观审查、另一个代理检查、避免上下文污染 | `independent_review_context_separation`、`read_only_audit_subagent_contract` 和 `agent_topology_selection_trace`，同会话角色扮演不得冒充独立评审 |
...
`user_input_classification_gate`：用户新增输入进入本轮 prompt 或计划前，先轻量分类为事实补充、偏好、授权、局部修正、目标变更或噪声/可能误导。事实补充进入 `current_basis` 等待证据分层；偏好影响交付风格；授权改变能力边界；局部修正只作为 prompt patch；只有目标变更或授权边界能改写主线。若用户输入只是中途疑问、局部情绪或可能误解，AI 应降低其对总规划的权重，先回到 `active_goal_summary`、`round_goal` 和证据状态判断。
...
108. **ask_user_overtrigger_gap**：`next_route`、`round_goal`、state 或材料已经给出明确、低风险、可逆下一步时，模型仍因礼貌、安全感或默认交互习惯停下问“是否继续”，打断连续推进。
109. **human_labor_offload_gap**：用户已经提供目录、文件、链接或材料位置，模型没有优先自行读取和归纳，反而要求用户整理 MD、复制粘贴全文、人工摘要或开新 session 承担 AI 能做的上下文搬运。
110. **same_session_independent_review_gap**：模型在同一聊天上下文里“扮演独立评审”并声称客观独立，忽略聊天历史和既有立场对审查的污染；真正独立评审应使用清上下文、新线程、只读审计子代理或事实账本降权输入。
111. **human_input_drift_gap**：用户局部反馈、疑问或误解被直接升级为目标变化，模型没有先按事实补充、偏好、授权、局部修正、目标变更、噪声/可能误导分类，导致主目标、master prompt 或本轮计划被人工输入带偏。
